...
export default function Reservasi() {
    ...
    const reservasi = () => {
        if (rsvpNama === '') {
            ...
            window.open(`https://api.whatsapp.com/send?phone=6285365650371&text=Hai%20ObidanIca%20%26%20May%2C%20saya%20*${rsvpNama}*%20ingin%20konfirmasi%20kehadiran%20pada%20undangan%20pernikahan%20kalian%20bahwa%20*${rsvpKonfir}*%20bersama%20*${rsvpTamu}*%20orang.%20Saya%20ucapkan%3A%0A*${rsvpPesan}*.%20Terima%20kasih%20ya.`, "_blank");
        }
    ...
    const [expiryTime, setExpiryTime] = useState("13 jan 2024 08:00:00");
    ...
    return (
        <>
            <div id="gift" className='my-16'>
                ...
                <h1 data-aos="fade-up" className='text-center text-2xl font-semibold custom-font-3 mx-4 text-[#477aa9]'>Menuju Waktu Acara</h1>
                <h1 data-aos="fade-up" className='text-center text-4xl font-semibold custom-font-3 mx-4 mb-8 text-[#477aa9]'>Pernikahan Kami</h1>
                <div data-aos="fade-up" className='flex justify-center gap-4 mb-8'>
                    <div className='rounded-xl md:w-[200px] w-[80px] py-6 bg-[#477aa9] text-center custom-font-3 text-white'>
                ...
                <h1 data-aos="fade-up" className='text-center text-2xl font-semibold custom-font-3 mx-4 text-[#477aa9] mb-4'>Untuk Yang Berhalangan Hadir, Kami Tambahkan Fitur Dibawah Ini</h1>
                <div data-aos="fade-up" className='flex justify-center'>
                    <button type="button" onClick={() => openAmplop()} className="text-white shadow-lg text-md bg-[#477aa9] hover:bg-[#31261d] font-bold rounded-xl px-5 py-3.5 mr-2 mb-2">
                        <FontAwesomeIcon icon={faMoneyBill1} /> Amplop
                    </button>
                    <button type="button" onClick={() => openKado()} className="text-white shadow-lg text-md bg-[#477aa9] hover:bg-[#31261d] font-bold rounded-xl px-5 py-3.5 mr-2 mb-2">
                        <FontAwesomeIcon icon={faGift} /> Kirim Kado
                    </button>
                </div>
            </div>
            <div className='mt-16 bg-[#477aa9] py-8'>
                ...
                <div data-aos="fade-up" className='max-w-lg mx-auto mb-16'>
                    <div className='mx-4'>
                        <input value={rsvpNama} onChange={(e) => { setRsvpNama(e.target.value) }} type="text" autoComplete='off' id="first_name" class="bg-gray-50 text-[#f5a525] border border-gray-300 font-bold text-sm rounded-lg focus:ring-[#fbeeee] focus:border-[#fbeeee] block w-full p-2.5 mb-2" placeholder="Isi Nama" />
                        <input type="text" value={rsvpTamu} onChange={(e) => { setRsvpTamu(e.target.value) }} autoComplete='off' id="first_name" class="bg-gray-50 text-[#f5a525] border border-gray-300 font-bold text-sm rounded-lg focus:ring-[#fbeeee] focus:border-[#fbeeee] block w-full p-2.5 mb-2" placeholder="Jumlah Tamu" />
                        <textarea id="message" value={rsvpPesan} onChange={(e) => { setRsvpPesan(e.target.value) }} rows="4" class="block p-2.5 w-full text-sm text-[#f5a525] font-bold bg-gray-50 rounded-lg border border-gray-300 focus:ring-[#fbeeee] focus:border-[#fbeeee] mb-2" placeholder="Pesan Ucapan">{rsvpPesan}</textarea>
                        ...
                        <button onClick={() => {reservasi()}} className='py-3 bg-green-600 rounded-full w-full mb-4 text-white hover:bg-green'>
                ...
                <div data-aos="fade-up" className='max-w-lg mx-auto mb-16'>
                    <div className='mx-4'>
                        <input autoComplete='off' type="text" value={formNama} onChange={(e) => { setFormNama(e.target.value) }} id="first_name" class="bg-gray-50 text-[#f5a525] border border-gray-300 font-bold text-sm rounded-lg focus:ring-[#fbeeee] focus:border-[#fbeeee] block w-full p-2.5 mb-2" placeholder="Isi Nama" />
                        <textarea autoComplete='off' id="message" rows="4" onChange={(e) => { setFormPesan(e.target.value) }} class="block p-2.5 w-full text-sm text-[#f5a525] font-bold bg-gray-50 rounded-lg border border-gray-300 focus:ring-[#fbeeee] focus:border-[#fbeeee] mb-2" value={formPesan} placeholder="Pesan"></textarea>
                        ...
                        <div className='max-h-[500px] overflow-auto px-2'>
                            {ucapan.map((ucap) => (
                                <div className='rounded-3xl rounded-bl-none bg-white p-4 custom-font-4 mb-4'>
                                    <h1 className='text-[#f5a525] font-bold text-sm'>{ucap.data.nama}</h1>
